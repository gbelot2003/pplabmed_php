...
use Acme\Controller\Printer\Bases\PDFENGImage as PDFENG;
use App\HistopatologiasEng;

class HistoForm1EngImage
{
    ...
    {
        $ftitle = $data->serial . "-" . $data->fecha_biopcia->format('Y');
    ...
    protected function PrintBody(HistopatologiasEng $data, $pdf)
    {
        ...
        if (count($data->images) > 0 && count($data->images) <= 3) {
            ...
            if (count($data->images) == 3) {
                ...
                $pdf->Image(public_path() . "/img/histo/" . $data->images[0]->image_url, 150, 82, 55, 45, '', '', '', true, 300, '', false, false, 0, false, false, false);
                $pdf->SetXY(150,126);
                $pdf->Cell(60, 10, $data->images[0]->descripcion, 0,  0, 'L');

                $pdf->Image(public_path() . "/img/histo/" . $data->images[1]->image_url, 150, 136, 55, 45, '', '', '', true, 300, '', false, false, 0, false, false, false);
                $pdf->SetXY(150,179);
                $pdf->Cell(60, 10, $data->images[1]->descripcion, 0,  0, 'L');

                $pdf->Image(public_path() . "/img/histo/" . $data->images[2]->image_url, 150, 190, 55, 45, '', '', '', true, 300, '', false, false, 0, false, false, false);
                $pdf->SetXY(150,233);
